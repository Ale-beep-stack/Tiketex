...
# Importar funciones modernas (OPCIONAL - para degradado)
try:
    from estilos_modernos import aplicar_degradado_ventana
    DEGRADADO_DISPONIBLE = True
except ImportError:
    DEGRADADO_DISPONIBLE = False
    print("Modo clásico: estilos_modernos.py no encontrado")
...
class TicketGeneratorApp:
    def __init__(self, root):
        ...
        # ✨ APLICAR DEGRADADO MODERNO (si está disponible)
        if DEGRADADO_DISPONIBLE:
            aplicar_degradado_ventana(self.root)
            print("✓ Degradado aplicado correctamente")
    ...
    def crear_interfaz(self):
        ...
        self.container = tk.Frame(self.root, bg=self.colores["fondo"])
        self.container.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # ✨ Aplicar degradado al container también
        if DEGRADADO_DISPONIBLE:
            aplicar_degradado_ventana(self.container)
        ...
        # ========== FRAME 1: TICKETS ==========
        self.frame_tickets = tk.Frame(self.container, bg=self.colores["fondo"])
        self.frame_tickets.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # ========== FRAME 2: INVENTARIO ==========
        self.frame_inventario = tk.Frame(self.container, bg=self.colores["fondo"])
        self.frame_inventario.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # ========== FRAME 3: REPORTES ==========
        self.frame_reportes = tk.Frame(self.container, bg=self.colores["fondo"])
        self.frame_reportes.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))
        
        # ✨ Aplicar degradado a los frames principales
        if DEGRADADO_DISPONIBLE:
            aplicar_degradado_ventana(self.frame_tickets)
            aplicar_degradado_ventana(self.frame_inventario)
            aplicar_degradado_ventana(self.frame_reportes)
    ...
    def crear_interfaz_tickets(self):
        ...
        try:
            ...
            img_mas = img_mas.resize((30, 30), Image.Resampling.LANCZOS)
